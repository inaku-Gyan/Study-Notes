...
\begin{equation}
  a y'' + b y' + c y = r(t)
  \label{eq: non-homogeneous CC ODE}
\end{equation}
...
\begin{align*}
  P_n(t) &:= A_n t^n + A_{n-1} t^{n-1} + \cdots + A_0 \\
  Q_n(t) &:= B_n t^n + B_{n-1} t^{n-1} + \cdots + B_0 \\
  R_n(t) &:= C_n t^n + C_{n-1} t^{n-1} + \cdots + C_0
\end{align*}
...
\begin{center}
  \begin{tabular}{c @{\hspace{2cm}} c}
    ...
    \midrule
    \(P_n(t)\) & \(t^s Q_n(t)\) \\
    \(P_n(t) e^{\alpha t}\) & \(t^s Q_n(t) e^{\alpha t}\) \\
    \bottomrule
    \end{tabular}
\end{center}
