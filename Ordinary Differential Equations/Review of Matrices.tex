...
\begin{proof}
  ...
  Since \(\dim(\Null(\mat{A} - \lambda \mat{I}_n)) = k\),
  we can pick \(k\) orthonormal eigenvectors
  \( \{\vec{v}_1, \vec{v}_2, \ldots, \vec{v}_k\} \) from \(\Null(\mat{A} - \lambda \mat{I}_n)\),
  ...
  \begin{align*}
    p(x) =
    \det(\mat{A} - x \mat{I}_n)
    &= \det(\mat{V}^\top (\mat{A} - x \mat{I}_n) \mat{V} ) \\
    &= \det(\mat{V}^\top \mat{A} \mat{V} - x \mat{I}_n) \\
    &= \det\left(
      \begin{bmatrix}
        & & \\
